...
struct _EFI_GRAPHICS_OUTPUT_PROTCOL;//ビデオ モードを設定し、グラフィックス コントローラーのフレーム バッファーとの間でピクセルをコピーするための基本的な抽象化を提供しま
struct _EFI_PIXEL_BITMASK;//ビットのカラーのマスク値
struct _EFI_GRAPHICS_OUTPUT_MODE_INFORMATION;//バッファのオプション値
struct _EFI_GRAPHICS_OUTPUT_PROTOCOL_MODE;//ピクセルをフレーム バッファーに直接描画できるようにするソフトウェア抽象化を提供します
struct _EFI_GRAPHICS_OUTPUT_BLT_PIXEL;//ピクセルのカラー情報です
struct _EFI_EDID_DISCOVERED_PROTOCOL;//このプロトコルには、ビデオ出力デバイスから取得された EDID 情報が含まれています。
struct _EFI_EDID_ACTIVE_PROTOCOL;//このプロトコルには、アクティブなビデオ出力デバイスの EDID 情報が含まれています。これは、オーバーライドが利用可能な場合は EFI_EDID_OVERRIDE_PROTOCOL から取得された EDID 情報、オーバーライドが利用できない場合は EFI_EDID_DISCOVERED_PROTOCOL からの EDID 情報の同一コピーのいずれかです。
struct _EFI_EDID_OVERRIDE_PROTOCOL;//このプロトコルは、プラットフォームがグラフィックス出力プロトコルのプロデューサーに EDID 情報を提供できるようにするためにプラットフォームによって生成されます。
